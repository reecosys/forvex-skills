...
| `forvex_emit_event` | **0.8.0** / **0.11.0** | Cross-lane ledger write. `entity_type`: `property` \| `deal` \| `workspace`. Lanes: `cmo` \| `cfo` \| `pm` \| `mkt` \| `ops` \| `net` \| `cos`. |
...
| `readvise_list_property_capex` | **0.10.0** | CapEx watch list (system, door, timing, $ range). Not Track capital_pool. |
| `readvise_upsert_property_capex` | **0.10.0** | Upsert CapEx watch item (match by id or property+system). Planning estimates only. |
| `readvise_list_rental_ops` | **0.10.0** | Monthly rental ops snapshots (rent, PM fee, mx, taxes, insurance). |
| `readvise_upsert_rental_ops` | **0.10.0** | Upsert one statement month per door (first-of-month key). |
